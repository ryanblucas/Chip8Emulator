...
static void window_handle_events(sf::RenderWindow& sfml_window, window& window)
{
...
void window::thread_proc()
{
	...
	while (window.isOpen())
	{
		window_handle_events(window, *this);
		window.clear();
		window_render_screen(window, *this);
		window.display();
	}
	m_window_thread_open = false;
}